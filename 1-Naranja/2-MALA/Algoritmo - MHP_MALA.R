MHP_MALA <- function(datos,
  ...
  log_ver <- fun_log_verosimilitud(theta, ea = cbind(phi_1, phi_2), bmarray, umat)
  ...
  for (i in 2:(iteraciones+periodo_quemado)) {
    ...
    # Ejecutar el algoritmo MALA
    MALA_valores <- MALA(theta[B1], Sigma[B1,B1], eps, phi_1, phi_2)
    
    # Cadena de propuesta para los hiperparametros
    propuesta[B1] <- MALA_valores$prop
    
    # Restricciones
    theta_prop <- theta_ut(propuesta)
    restriccion_1 <- (theta_prop[1] > 150  & theta_prop[1] < 200)
    restriccion_2 <- (theta_prop[2] > 10  & theta_prop[2] < 50)
    restriccion_3 <- (theta_prop[3] > 300  & theta_prop[3] < 375)
    restriccion_4 <- (theta_prop[4] > 25  & theta_prop[4] < 75)
    ...
    if (restricciones == 0) {
      ...
      # Logaritmo de las densidad de transicion (no simetricas log_q no es igual a log_q_star)
      log_q <- MALA_valores$log_q
      log_q_star <- MALA_valores$log_q_star
      
      # Calcular el logaritmo de la densidad a posteriori - hiperparametros
      log_post <- log_eta(phi_1, phi_2, theta) + log_prior(theta)
      
      # Calcular el logaritmo de la densidad a posteriori (nuevo) - hiperparametros  
      log_post_nuevo <- log_eta(phi_1, phi_2, propuesta) + log_prior(propuesta)
      
      # Ratio de aceptacion
      MHRatio <- min(0, log_post_nuevo - log_post + log_q_star - log_q)  
      }
