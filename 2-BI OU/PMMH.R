...
par(mfrow=c(2,2))
for (k in 1:4) {
  boxplot(res$ReChain[,k,],main="", xlab=parameterNamesRe[k],
          ylab="", col="pink")  
}
...
parameterNames <- c(expression(beta[11]), expression(beta[12]),
                    ...
                    expression(xi[1]), expression(xi[2]))
...
parameterACFnames <- c(expression("ACF de " *beta[11]), expression("ACF de " *beta[12]),
                       expression("ACF de " *beta[12]), expression("ACF de " *beta[22]),
                       expression("ACF de " *alpha[1]), expression("ACF de " *alpha[2]),
                       expression("ACF de " *sigma[1]), expression("ACF de " *sigma[2]),
                       expression("ACF de " *xi[1]) ,expression("ACF de " *xi[2])
)
...
parameterNamesHyper <- c(expression(nu[11]), expression(nu[12]),
                         expression(nu[21]), expression(nu[22]))

par(mfrow=c(2,2))
for (k in 1:4) {
  boxplot(res$HyperChain[,k,],main="",
          xlab=parameterNamesHyper[k],
          ylab="", col=c("red","blue"))  
}

colMeans(res$HyperChain[,4,]) %>% round(.,2)
colSds(res$HyperChain[,4,]) %>% round(.,2)

fit_MCMC <- as.mcmc(res$ThetaChain)
p <- c(
  res$mcmc.time, mean(effectiveSize(fit_MCMC)),
  mean(effectiveSize(fit_MCMC)) / res$mcmc.time,
...
# save(res, file = "rescorrMetropolis.RData")
